function az = zoomPlot(ah, source_pos, target_pos, display_x_axis)
...
% Draw lines from box to section
distance_from_top_of_box = abs(ty2 - source_pos(2));
distance_from_bottom_of_box = abs(ty1 - source_pos(4));
if distance_from_top_of_box < distance_from_bottom_of_box
    line([tx1 source_pos(1)],[ty2 source_pos(2)] ,'Color','black','LineStyle',':', 'HandleVisibility','off')
    line([tx2 source_pos(3)],[ty2 source_pos(2)] ,'Color','black','LineStyle',':', 'HandleVisibility','off')
else
    line([tx1 source_pos(1)],[ty1 source_pos(4)] ,'Color','black','LineStyle',':', 'HandleVisibility','off')
    line([tx2 source_pos(3)],[ty1 source_pos(4)] ,'Color','black','LineStyle',':', 'HandleVisibility','off')
end
